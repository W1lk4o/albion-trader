...
(function () {
  ...
  const MARKET_RULES = {
    maxSellAgeHours: 24,
    maxBuyAgeHours: 24,
    maxOpportunityAgeHours: 18,
    lowOutlierRatio: 0.45,
    highOutlierRatio: 2.2,
    minSpreadSilver: 150,
    maxSpreadPctForAutoAccept: 250,
    minCitiesForOpportunity: 2
  };
  ...
  const ITEM_CATALOG = {
    ...
    'Armadura de placa': {
      'Capuzes e elmos': [
        { label: 'Capacete de soldado', template: 'T{tier}_HEAD_PLATE_SET1' },
        { label: 'Capacete de guardião', template: 'T{tier}_HEAD_PLATE_SET2' },
        { label: 'Capacete de cavaleiro', template: 'T{tier}_HEAD_PLATE_SET3' }
      ],
      'Armaduras': [
    ...
    'Armas': {
      'Espadas': [
        { label: 'Espada', template: 'T{tier}_MAIN_SWORD' },
        { label: 'Claymore', template: 'T{tier}_2H_CLAYMORE' },
        { label: 'Espadas duplas', template: 'T{tier}_2H_DUALSWORD' }
      ],
      'Machados': [
        { label: 'Machado de batalha', template: 'T{tier}_MAIN_AXE' },
        { label: 'Machado grande', template: 'T{tier}_2H_AXE' },
        { label: 'Alabarda', template: 'T{tier}_2H_HALBERD' }
      ],
      'Maças': [
        { label: 'Maça', template: 'T{tier}_MAIN_MACE' },
        { label: 'Maça pesada', template: 'T{tier}_2H_MACE' }
      ],
      'Martelos': [
        { label: 'Martelo', template: 'T{tier}_MAIN_HAMMER' },
        { label: 'Martelo de guerra', template: 'T{tier}_2H_HAMMER' }
      ],
      'Lanças': [
        { label: 'Lança', template: 'T{tier}_MAIN_SPEAR' },
        { label: 'Arpão', template: 'T{tier}_2H_HARPOON' },
        { label: 'Lança de guardião', template: 'T{tier}_MAIN_SPEAR_KEEPER' }
      ],
      'Arcos': [
      ...
      'Bestas': [
        { label: 'Besta', template: 'T{tier}_2H_CROSSBOW' },
        { label: 'Besta pesada', template: 'T{tier}_2H_CROSSBOWLARGE' }
      ],
      'Adagas': [
        { label: 'Adaga', template: 'T{tier}_MAIN_DAGGER' },
        { label: 'Pares de adagas', template: 'T{tier}_2H_DAGGERPAIR' }
      ],
      'Cajados de fogo': [
        { label: 'Cajado de fogo', template: 'T{tier}_MAIN_FIRESTAFF' },
        { label: 'Grande cajado de fogo', template: 'T{tier}_2H_FIRESTAFF' }
      ],
      'Cajados de gelo': [
        { label: 'Cajado de gelo', template: 'T{tier}_MAIN_FROSTSTAFF' },
        { label: 'Grande cajado de gelo', template: 'T{tier}_2H_FROSTSTAFF' }
      ],
      'Cajados sagrados': [
        { label: 'Cajado sagrado', template: 'T{tier}_MAIN_HOLYSTAFF' },
        { label: 'Grande cajado sagrado', template: 'T{tier}_2H_HOLYSTAFF' }
      ],
      'Cajados da natureza': [
        { label: 'Cajado da natureza', template: 'T{tier}_MAIN_NATURESTAFF' },
        { label: 'Grande cajado da natureza', template: 'T{tier}_2H_NATURESTAFF' }
      ],
      'Cajados amaldiçoados': [
        { label: 'Cajado amaldiçoado', template: 'T{tier}_MAIN_CURSEDSTAFF' },
        { label: 'Grande cajado amaldiçoado', template: 'T{tier}_2H_CURSEDSTAFF' }
      ],
      'Cajados arcanos': [
        { label: 'Cajado arcano', template: 'T{tier}_MAIN_ARCANESTAFF' },
        { label: 'Grande cajado arcano', template: 'T{tier}_2H_ARCANESTAFF' }
      ]
    },
    'Mão secundária': {
      'Escudos e apoio': [
        { label: 'Escudo', template: 'T{tier}_OFF_SHIELD' },
        { label: 'Tocha', template: 'T{tier}_OFF_TORCH' },
        { label: 'Livro', template: 'T{tier}_OFF_BOOK' },
        { label: 'Orbe', template: 'T{tier}_OFF_ORB_MORGANA' }
      ]
    },
    'Ferramentas': {
      'Coleta': [
        { label: 'Machado de coleta', template: 'T{tier}_TOOL_AXE' },
        { label: 'Picareta', template: 'T{tier}_TOOL_PICK' },
        { label: 'Foice', template: 'T{tier}_TOOL_SICKLE' },
        { label: 'Esfolador', template: 'T{tier}_TOOL_KNIFE' },
        { label: 'Martelo de pedreiro', template: 'T{tier}_TOOL_HAMMER' }
      ]
    },
    'Consumíveis': {
      'Comidas': [
        ...
        { label: 'Sanduíche', template: 'T{tier}_MEAL_SANDWICH' },
        { label: 'Torta', template: 'T{tier}_MEAL_PIE' }
      ],
      'Poções': [
        ...
        { label: 'Poção de gigantismo', template: 'T{tier}_POTION_GIGANTIFY' },
        { label: 'Poção de energia', template: 'T{tier}_POTION_ENERGY' }
      ]
    }
  };
  ...
  const POPULAR_ITEMS = Array.from(new Set([
    ...
    'T4_ORE','T5_ORE','T6_ORE','T4_HIDE','T5_HIDE','T6_HIDE','T4_ROCK','T5_ROCK','T6_ROCK',
  ...
  function getAllCatalogTemplates() {
    const templates = [];
    Object.values(ITEM_CATALOG).forEach((groups) => {
      Object.values(groups).forEach((items) => {
        items.forEach((item) => templates.push(item.template));
      });
    });
  ...
  function getAllCatalogItemIds() {
    const ids = [];
    getAllCatalogTemplates().forEach((template) => {
      for (let tier = 4; tier <= 8; tier += 1) {
        ids.push(buildItemId(template, tier, 0));
      }
    });
    return Array.from(new Set(ids));
  }

  function getScanItemsForMode(mode) {
    return mode === 'all' ? getAllCatalogItemIds() : POPULAR_ITEMS;
  }
  ...
  function isFreshWithinHours(date, maxHours) {
    return hoursSince(date) <= maxHours;
  }

  function sanitizeRows(rows, options = {}) {
    const maxSellAgeHours = options.maxSellAgeHours || MARKET_RULES.maxSellAgeHours;
    const maxBuyAgeHours = options.maxBuyAgeHours || MARKET_RULES.maxBuyAgeHours;
    ...
    grouped.forEach((cityRows, city) => {
      const validSellRows = cityRows
        .filter((row) => Number(row.sell_price_min || 0) > 0 && isFreshWithinHours(parseTime(row.sell_price_min_date), maxSellAgeHours))
        .sort((a, b) => {
          const priceDiff = Number(a.sell_price_min || 0) - Number(b.sell_price_min || 0);
          if (priceDiff !== 0) return priceDiff;
          return (parseTime(b.sell_price_min_date)?.getTime() || 0) - (parseTime(a.sell_price_min_date)?.getTime() || 0);
        });

      const validBuyRows = cityRows
        .filter((row) => Number(row.buy_price_max || 0) > 0 && isFreshWithinHours(parseTime(row.buy_price_max_date), maxBuyAgeHours))
        .sort((a, b) => {
          const priceDiff = Number(b.buy_price_max || 0) - Number(a.buy_price_max || 0);
          if (priceDiff !== 0) return priceDiff;
          return (parseTime(b.buy_price_max_date)?.getTime() || 0) - (parseTime(a.buy_price_max_date)?.getTime() || 0);
        });

      const base = cityRows[0] || {};
      const bestSell = validSellRows[0];
      const bestBuy = validBuyRows[0];

      mergedByCity.push({
        ...base,
        city,
        item_id: base.item_id || bestSell?.item_id || bestBuy?.item_id,
        quality: base.quality || bestSell?.quality || bestBuy?.quality,
        sell_price_min: bestSell?.sell_price_min || 0,
        sell_price_min_date: bestSell?.sell_price_min_date || null,
        buy_price_max: bestBuy?.buy_price_max || 0,
        buy_price_max_date: bestBuy?.buy_price_max_date || null
      });
    });

    const validSellValues = mergedByCity.map(r => Number(r.sell_price_min || 0)).filter(v => v > 0);
    const validBuyValues = mergedByCity.map(r => Number(r.buy_price_max || 0)).filter(v => v > 0);
    const sellMedian = median(validSellValues);
    const buyMedian = median(validBuyValues);

    return mergedByCity.filter((row) => {
      ...
      const sellFresh = sell > 0 && isFreshWithinHours(sellDate, maxSellAgeHours);
      const buyFresh = buy > 0 && isFreshWithinHours(buyDate, maxBuyAgeHours);

      const sellOk = sellFresh && (!sellMedian || (sell >= sellMedian * MARKET_RULES.lowOutlierRatio && sell <= sellMedian * MARKET_RULES.highOutlierRatio));
      const buyOk = buyFresh && (!buyMedian || (buy >= buyMedian * MARKET_RULES.lowOutlierRatio && buy <= buyMedian * MARKET_RULES.highOutlierRatio));

      if (sellOk && buyOk && buy > sell * (1 + MARKET_RULES.maxSpreadPctForAutoAccept / 100)) return false;
      return sellOk || buyOk;
    });
  }
  ...
  function confidenceLabel(metrics) {
    const spreadPct = Number(metrics?.spreadPct || 0);
    const validCities = Number(metrics?.validCities || 0);
    const spreadSilver = Number(metrics?.spreadSilver || 0);
    const buyHours = Number(metrics?.buyHours || Infinity);
    const sellHours = Number(metrics?.sellHours || Infinity);

    if (validCities >= 4 && spreadPct >= 6 && spreadSilver >= 150 && buyHours <= 8 && sellHours <= 8) return 'Alta';
    if (validCities >= 3 && spreadPct >= 3 && spreadSilver >= 100 && buyHours <= 16 && sellHours <= 16) return 'Boa';
    if (validCities >= 2 && spreadPct >= 1.5 && spreadSilver >= 50) return 'Média';
    return 'Baixa';
  }
  ...
  function buildSingleItemAnalysis(rows, feePct = DEFAULT_FEE, options = {}) {
    const cleaned = sanitizeRows(rows, {
      maxSellAgeHours: options.maxSellAgeHours || MARKET_RULES.maxSellAgeHours,
      maxBuyAgeHours: options.maxBuyAgeHours || MARKET_RULES.maxBuyAgeHours
    });
    ...
    if (!validSells.length || !validBuys.length) {
      return { ok: false, reason: 'Os preços vieram velhos demais, fora da curva ou sem oferta/compra útil.' };
    }
    ...
    const buyAgeHours = hoursSince(parseTime(cheapest.sell_price_min_date));
    const sellAgeHours = hoursSince(parseTime(highest.buy_price_max_date));
    const confidence = confidenceLabel({
      spreadPct: margin,
      spreadSilver: profit,
      validCities: cleaned.length,
      buyHours: buyAgeHours,
      sellHours: sellAgeHours
    });

    if (grossSpread <= 0 || profit <= 0) {
      return { ok: false, reason: 'Não apareceu spread líquido positivo e plausível para esse item.' };
    }

    if (profit < MARKET_RULES.minSpreadSilver && confidence === 'Baixa') {
      return { ok: false, reason: 'Spread pequeno demais para considerar como oportunidade confiável.' };
    }
    ...
    return {
      ...
      confidence,
      validSellCities: validSells.length,
      validBuyCities: validBuys.length
    };
  }
  ...
  function buildOpportunities(prices, capital = 3000000, profile = 'balanced', feePct = DEFAULT_FEE, route = 'safe') {
    ...
    byItem.forEach((rows, itemId) => {
      const analysis = buildSingleItemAnalysis(rows, feePct, {
        maxSellAgeHours: MARKET_RULES.maxOpportunityAgeHours,
        maxBuyAgeHours: MARKET_RULES.maxOpportunityAgeHours
      });
      if (!analysis.ok) return;
      if (analysis.buyCity === analysis.sellCity) return;
      if (analysis.cleaned.length < MARKET_RULES.minCitiesForOpportunity) return;
  ...
  async function loadMarket() {
    ...
    try {
      ...
      const tableRows = analysis.cleaned.map((row) => {
        const sell = Number(row.sell_price_min || 0);
        const buy = Number(row.buy_price_max || 0);
        const askValid = sell > 0 && isFreshWithinHours(parseTime(row.sell_price_min_date), MARKET_RULES.maxSellAgeHours);
        const buyValid = buy > 0 && isFreshWithinHours(parseTime(row.buy_price_max_date), MARKET_RULES.maxBuyAgeHours);
        return `
          <tr>
            <td>${row.city}</td>
      ...
      box.innerHTML = `
        <div class="market-summary">
          ...
          <div><span class="label">Atualização da melhor compra:</span> ${formatBrazilTime(analysis.buyUpdatedAt)}</div>
          <div><span class="label">Atualização da melhor venda:</span> ${formatBrazilTime(analysis.sellUpdatedAt)}</div>
        </div>
  ...
  async function loadOpportunityRadar(mode = 'popular') {
    ...
    const items = getScanItemsForMode(mode);
